...
class MyFrame(wx.Frame):
    ...
    def setPlayer(self):
        ...
        #self._playerAI = Player.DeepQNetWork(not(self._SenteColor),"AI_DQN.pkl")
        self._playerAI = Player.DNet(not(self._SenteColor),"AI_DQN.pkl")
        #self._playerAI = Player.DNet(not(self._SenteColor),"DQN.pkl")
    ...
    def selectMenu(self, event):
        ...
        if(event.GetId() == 1):
            print("保存")
            self._playerAI.save_network("AI_DQN.pkl")
        if(event.GetId() == 7):
