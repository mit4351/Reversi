...
def maxtone(board, _color, all_candidatess):
    filtered_candidates = []
    max_score = -1
    for candidates in all_candidatess:
        next_board = put_stone(board.stone_status, _color, candidates[0], candidates[1])
        score = get_score(board.stone_status, _color)
        if score >= max_score:
...
def PointsToMask(pPoints):
    ...
    m = np.random.uniform(low=0.1, high=0.2, size=(65))
