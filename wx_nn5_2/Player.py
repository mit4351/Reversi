...
class DNet(Player):
    ...
    def update_network(self, pPoint=0, learning_rate=0.001):
        ...
        for i in range(0,len(self._history)):
            ...
            wMaxPoint =  self._history[i][10]
            all_candidates  = self._history[i][11]
            self.back_propagation(x, z1, z2, z3, z4, y, p, m3, m4, msk, wMaxPoint, all_candidates, i+1, pPoint)
    ...
    def next_move(self, board):
        ...
        self._history.append([x, y, z1, z2, z3, z4, p, m3, m4, msk, wMaxPoint, all_candidates])
    ...
    # 誤差逆伝播
    def back_propagation(self, x, z1, z2, z3, z4, y, p, m3, m4, msk, wMaxPoint, all_candidates, i, pPoint, learning_rate=0.001):
        W1,W2,W3,W4,W5 = self.network['W1'], self.network['W2'], self.network['W3'], self.network['W4'], self.network['W5']
        #y = softmax(pY)
        t = np.zeros(len(y))
        if pPoint >= 0:
