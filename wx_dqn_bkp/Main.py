...
def Training(pCount=10, pOutput=True, epoch=2):
    ...
    #white_player = Player.DeepQNetWork(ReverseCommon.WHITE,"AI_Deep.pkl")
    white_player = Player.QNetWork(ReverseCommon.WHITE,"AI_Deep.pkl")
    cnt = 0
    lap = 0
    start = 0
    for j in range(0, epoch):
        ...
        for i in range(0, pCount):
            ...
            cnt = white_player.update_network(player2_score - 32)
            if i==0:
                lap = cnt
            if j==0 and i==0:
                start = cnt
    ...
    plt.title("DQN (Start:%5d  End:%5d)"  % (start, cnt))
