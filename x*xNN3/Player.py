...
""" プレーヤの基盤クラス(AIも含む) """
class Player:
    def __init__(self, color, bord_size):
        """ コンストラクタ """
        self._color = color
...
class RandomAiKnowGoodMove(Player):
    def next_move(self, board):
        a = int(self._bord_size) -1
        known_good_moves = [[0, 0], [0, a], [a, 0], [a, a]]
        all_candidates = ReverseCommon.get_puttable_points(board.stone_status, self._color)
        # 4隅が取れるなら取る
...
class NNetWork(Player):
    def __init__(self, color, bord_size, filename, train_flg=False):
        super(NNetWork, self).__init__(color, bord_size)
    ...
    def init_wait(self):
        a = int(self._bord_size)
        n_in = a * a   #4*4 箇所
        n_hiddn1 = 1000 #300  #100
        n_hiddn2 = 1000
        n_out = n_in + 1  #4*4 箇所 + 1(パス)
    ...
    def brain(self,x,all_candidates, board):
        msk = ReverseCommon.PointsToMask(all_candidates, self._bord_size)
        ...
        wPoint = ReverseCommon.IndexToPoint(p, self._bord_size)
    ...
    def back_propagation(self, x, z1, z2, y, p, msk, wMaxPoint, all_candidates, i, pPoint, learning_rate=0.001):
        ...
        if pPoint >= 0:
            ...
            #print("mp=%s" %(mp))
            mp = wMaxPoint
            cp = ReverseCommon.PointToIndex(mp, self._bord_size)
            t[cp] = 1
