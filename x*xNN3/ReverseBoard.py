...
""" オセロ盤 """
class ReverseBoard:
    def __init__(self, bord_size):
        """ Constructor """
        # ボード初期化
        p = int(bord_size)
        a = int(bord_size /2)
        b = int(a -1)
        self._stone_status = [[ReverseCommon.NONE for i in range(p)] for j in range(p)]
        self._stone_status[b][b] = ReverseCommon.WHITE
        self._stone_status[a][a] = ReverseCommon.WHITE
        self._stone_status[b][a] = ReverseCommon.BLACK
        self._stone_status[a][b] = ReverseCommon.BLACK
        # 黒のターンに初期化
