...
class MyFrame(wx.Frame):
    def __init__(self, bord_size, filename, *args, **kw):
        super(MyFrame, self).__init__(*args, **kw)
        self._bord_size = bord_size
        ...
        self.SetMyMenuBar()
        self.InitUI(bord_size, bord_size)
        self.setPlayer()
        self.Show()

    def InitUI(self, pRow, pCol):
    ...
    def setStoneColor(self):
        ...
        for pos in all_candidates:
            self.Labels[str(ReverseCommon.PointToIndex(pos, self._bord_size))].SetForegroundColour(wx.RED)
    ...
    def setPlayer(self):
        ...
        self._playerAI = Player.NNetWork(not(self._SenteColor), self._bord_size,self._filename)
        self._reverse_board = ReverseBoard.ReverseBoard(self._bord_size)
    ...
    def StoneClick(self, event):
        ...
        # 置ける場所か確認
        if  not ReverseCommon.isPuttable_points(self._reverse_board, ReverseCommon.IndexToPoint(click.GetId(),self._bord_size)):
            return

        #人間手番反映
        self._reverse_board.put_stone2(ReverseCommon.IndexToPoint(click.GetId(), self._bord_size))
        self.setStoneColor()
...
def DisplayBorde(bord_size, filename):
    app = wx.App()
    MyFrame(bord_size, filename,None).Show(True)
    app.MainLoop()

if __name__ == '__main__':
    DisplayBorde(8,"AI_QNetWork.pkl")
